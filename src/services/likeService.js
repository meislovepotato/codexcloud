...
export const checkLikeStatus = async (postId) => {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/like/status/${postId}`);
...
export const toggleLike = async (postId) => {
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/like/toggle/${postId}`);
